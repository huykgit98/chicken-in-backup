...
class DashboardPage extends React.Component {
  ...
  async componentDidMount () {
    try {
      ...
      const response = await axios.get(`https://${hostURL || window.location.host}/api/shifts/employee`)
  ...
  archiveRejectedShift = async (shiftId) => {
    try {
      ...
      const result = await axios.put(`https://${hostURL || window.location.host}/api/shifts/archive/${shiftId}`)
  ...
  deletePendingShift = async (shiftId) => {
    try {
      // 1. Send the delete request to the server
      const result = await axios.delete(`https://${hostURL || window.location.host}/api/shifts/delete/${shiftId}`)
  ...
  addShift = async (newShiftObject) => {
    try {
      ...
      const result = await axios.post(`https://${hostURL || window.location.host}/api/shifts/create`, {
